 ...
 * stage 08 (second œuvre). Dézoome la caméra et force le peuplement des chunks
 * pour voir la COMPOSITION (fourgon artisan NE / zones cloisons arc O-SO /
 * landmark partition Nord) plutôt que le petit cadre de spawn.
 */
test('golden overview — arène composée stage 08 second_oeuvre', async ({ page }) => {
  await page.goto('/?autostart=solo&level=8&seed=1&test=1')
  ...
  await page.screenshot({ path: 'test-results/golden-stage08-overview.png' })
