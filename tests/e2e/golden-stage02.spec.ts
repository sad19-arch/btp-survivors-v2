...
/**
 * Checkpoint visuel : stage 02 (terrassement) avec composition scriptée.
 * Charge le stage 02, laisse le monde se poser (quelques frames simulées),
 * fait une capture PNG dans test-results/golden-stage02.png.
 *
 ...
 *
 * Le test ne plante PAS si le dossier test-results n'existe pas —
 * Playwright le crée automatiquement.
 */
test('golden stage 02 — terrassement — charge sans crash et produit une capture', async ({ page }) => {
  // Autostart sur le stage 02 (level=2), seed fixe pour reproductibilité.
  ...
  // Capture : le nom est absolu pour que Playwright la place dans test-results/.
  const capturePath = path.join('test-results', 'golden-stage02.png')
  await page.screenshot({ path: capturePath, fullPage: false })

  // Vérification minimaliste : le fichier a bien été écrit (taille > 0 implicite).
  console.log(`[golden-stage02] capture => ${capturePath}`)
})

/**
 * Vérifie que le stage 02 est stable pendant 10 s simulées (pas de NaN,
 * pas de crash, scène = game ou upgrade).
 */
test('golden stage 02 — stable 10 s', async ({ page }) => {
