 ...
 * stage 06 (échafaudages). Dézoome la caméra et force le peuplement des chunks
 * pour voir la COMPOSITION (nacelle NE / grilles de cadres arc O-S /
 * landmark tour d'échafaudage Nord) plutôt que le petit cadre de spawn.
 */
test('golden overview — arène composée stage 06 echafaudages', async ({ page }) => {
  await page.goto('/?autostart=solo&level=6&seed=1&test=1')
  ...
  await page.screenshot({ path: 'test-results/golden-stage06-overview.png' })
