...
describe('mergeFrames', () => {
  it('somme les composantes de move', () => {
    const a: FrameInput = { move: { x: 0.3, y: -0.2 }, pressed: [], action: false }
    const b: FrameInput = { move: { x: 0.1, y: 0.1 }, pressed: [], action: false }
  ...
  it('clampe move à [-1, 1] sans laisser dépasser (0.8 + 0.8 = 1, pas 1.6)', () => {
    const a: FrameInput = { move: { x: 0.8, y: -0.8 }, pressed: [], action: false }
    const b: FrameInput = { move: { x: 0.8, y: -0.8 }, pressed: [], action: false }
    const merged = mergeFrames(a, b)
  ...
  it('fait l’union dédupliquée de pressed (pas de doublon)', () => {
    const a: FrameInput = { move: { x: 0, y: 0 }, pressed: ['down', 'confirm'], action: false }
    const b: FrameInput = { move: { x: 0, y: 0 }, pressed: ['down'], action: false }
    const merged = mergeFrames(a, b)
  ...
  it('ne mute pas les entrées', () => {
    const a: FrameInput = { move: { x: 0.1, y: 0.1 }, pressed: ['up'], action: false }
    const b: FrameInput = { move: { x: 0.1, y: 0.1 }, pressed: ['up'], action: false }
    const aCopy = { move: { ...a.move }, pressed: [...a.pressed], action: a.action }
    const bCopy = { move: { ...b.move }, pressed: [...b.pressed], action: b.action }
    mergeFrames(a, b)
    expect(a).toEqual(aCopy)
    expect(b).toEqual(bCopy)
  })

  it('action = OU logique (l’un des deux tient l’action → fusion tient l’action)', () => {
    const a: FrameInput = { move: { x: 0, y: 0 }, pressed: [], action: true }
    const b: FrameInput = { move: { x: 0, y: 0 }, pressed: [], action: false }
    expect(mergeFrames(a, b).action).toBe(true)
    expect(mergeFrames(b, a).action).toBe(true)
    expect(mergeFrames(b, b).action).toBe(false)
  })
})
...
describe('buildPlayerInputs', () => {
  it('solo (playerCount=1) : map a exactement la clé 1 = fusion clavier + pads[0]', () => {
    ...
    const pads: FrameInput[] = [{ move: { x: 0, y: 0 }, pressed: ['down'], action: false }]
  ...
  it('playerCount=0 : la clé 1 existe TOUJOURS (max(,1)) = kb⊕pad0 — nav menu au titre', () => {
    const keyboard: FrameInput = { move: { x: 0, y: 1 }, pressed: ['confirm'], action: false }
    const pads: FrameInput[] = [{ move: { x: 0, y: 0 }, pressed: [], action: false }]
  ...
  it('coop4 (playerCount=4) : clés {1,2,3,4}, id1 = fusion, 2/3/4 = pads verbatim (moves distincts)', () => {
    const keyboard: FrameInput = { move: { x: 0, y: 0 }, pressed: [], action: false }
    const pads: FrameInput[] = [
      { move: { x: 0.1, y: 0 }, pressed: [], action: false },
      { move: { x: 0.2, y: 0 }, pressed: [], action: false },
      { move: { x: 0.3, y: 0 }, pressed: [], action: false },
      { move: { x: 0.4, y: 0 }, pressed: [], action: false },
    ]
  ...
  it('propage action=true du pad k≥2 verbatim (releveur en coop)', () => {
    const keyboard: FrameInput = { move: { x: 0, y: 0 }, pressed: [], action: false }
    const pads: FrameInput[] = [
      { move: { x: 0, y: 0 }, pressed: [], action: false },
      { move: { x: 0, y: 0 }, pressed: [], action: true },
    ]
