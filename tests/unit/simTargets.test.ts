...
/** Profil kite PASSANT par défaut (tendu mais gagnable) ; surcharger pour tester un échec. */
function agg(partial: Partial<BotAggregate>): BotAggregate {
  return {
    bot: 'kite', runs: 10, survivedFullPct: 20, survivalMsMedian: 220000,
    survivalMsMin: 70000, survivalMsMax: 600000, levelAt5minMedian: 11,
    peakEnemiesMedian: 50, bucketSec: 10, hpPctCurve: [100, 80, 40, 55], enemiesCurve: [], ...partial
  }
}

describe('evaluateTargets (tendu mais gagnable)', () => {
  ...
  it('PASS quand les PV de kite plongent sous 45% (climax)', () => {
    const rep = evaluateTargets([agg({ bot: 'kite', hpPctCurve: [100, 80, 40, 60] })])
    expect(rep.pass).toBe(true)
  })

  it('FAIL quand les PV de kite ne descendent jamais sous 45% (trop sûr)', () => {
  ...
  it('FAIL si greedy survit la run pleine (imprudent = doit mourir)', () => {
    const rep = evaluateTargets([agg({ bot: 'greedy', survivedFullPct: 100, survivalMsMedian: 600000 })])
    expect(rep.pass).toBe(false)
    expect(rep.failures.join(' ')).toContain('greedy')
  })

  it('FAIL si idle survit la run pleine (immobile = doit mourir)', () => {
    const rep = evaluateTargets([agg({ bot: 'idle', survivedFullPct: 100, survivalMsMedian: 600000 })])
    expect(rep.pass).toBe(false)
    expect(rep.failures.join(' ')).toContain('idle')
  })

  it('FAIL si greedy meurt dans les toutes premières secondes (< 40s)', () => {
