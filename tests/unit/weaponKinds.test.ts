...
function addPlayer(w: World, weaponId: string): EntityId {
  ...
  w.add(e, 'weapons', { slots: [{ id: weaponId, cooldownLeftMs: 0 }] })
  return e
}
...
describe('arme aura (marteau)', () => {
  it('inflige des dégâts aux ennemis dans le rayon de l’onde', () => {
    const w = new World()
    addPlayer(w, 'marteau')
    const near = addEnemy(w, 100, 0) // dans le rayon (120)
...
describe('arme orbitale (scie)', () => {
  ...
  it('blesse un ennemi situé sur la trajectoire des lames', () => {
    const w = new World()
    addPlayer(w, 'scie')
    const enemy = addEnemy(w, 72, 0) // sur le cercle d'orbite (rayon 72)
