...
3bis. **🔴 Vérif complétude des vues AVANT d'animer** — obligatoire pour tout personnage/véhicule à 4 directions (walk sheet). Mesurer la **bbox des pixels opaques** des 8 rotations générées et confirmer : `south ≈ north` (axe long **vertical**), `east ≈ west` (axe long **horizontal**, miroir), et qu'**au moins une vue montre le sujet de face** (pas seulement de dos/profil). **Si une vue cardinale manque ou est en réalité une élévation de côté déguisée, régénérer le personnage avant d'appeler `animate_character`** — ne PAS dépenser le budget d'animation sur un set incomplet. (Incident vécu : un camion généré par `create_character(v3, reference)` n'avait de vue de face sur AUCUNE des 8 rotations — irrécupérable, régénération complète nécessaire. La méthode bbox l'aurait détecté en 1 minute, avant l'animation.)
